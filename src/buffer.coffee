...
exports.Buffer = class BaseBuffer
  ...
  @decode : (s, enc) ->
    switch enc
      when 'binary'  then @make().binary_decode s
      when 'base64'  then @make().base64_decode s
      when 'base64a' then @make().base64a_decode s
      when 'base64x' then @make().base64x_decode s
      when 'base32'  then @make().base32_decode s
      when 'hex'     then @make().base16_decode s
      when 'ui8a'    then @make().ui8a_decode s
